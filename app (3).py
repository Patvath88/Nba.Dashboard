...
def search_players(query: str, api_key: str) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        # Catch any network or connection errors to avoid crashing the app
...
def fetch_stats(
    ...
    while True:
        ...
        try:
            ...
            break
        if resp.status_code != 200:
            # A 401 status typically indicates an invalid or missing API key.
            # Surface a single error message to the user rather than
            # repeating it across multiple calls.
...
def fetch_season_average(
    ...
    try:
        ...
        return None
    if resp.status_code != 200:
        # Surface a warning instead of repeated errors; the user may
        # need to verify their API key or plan.  Season averages are
        # non‑critical to the dashboard since we compute per‑game
        # averages from the stats endpoint.
        st.warning(
