# -------------------------------------------------
# HOT SHOT PROPS — NBA AI DASHBOARD (TEAM LOGO + RECENT FORM PATCH, FINAL)
# -------------------------------------------------
...
.team-bg {position:fixed;top:0;left:0;width:100%;height:100%;
          ...
          background-size:550px;opacity:0.05;z-index:-1;filter:grayscale(40%);}
...
def get_player_photo(player_name):
    """Fallback-safe headshot from NBA CDN."""
    first, last = player_name.lower().split(" ", 1)
    return f"https://cdn.nba.com/headshots/nba/latest/1040x760/{first}-{last}.png"

def get_team_logo(team_name):
    """Attempts both Sports Reference and ESPN CDN logos."""
    slug = team_name.lower().replace(" ", "-").replace("state-", "")
    return f"https://cdn.ssref.net/req/202305101/images/team_logos/{slug}.png"

def avg_stats(df):
    return {s: round(df[s].mean(), 1) for s in ["PTS","REB","AST","FG3M","STL","BLK","TOV","PRA"] if s in df.columns}
...
# Display section: last game validation
...
if not bt.empty:
    ...
    metric_cards(pred)
    diff = {k: round(pred[k] - actual.get(k, 0), 1) for k in pred if k in actual}
    bar_chart(diff, "Model Deviation (Predicted - Actual)", color="#ff6f00")
...
for title, df in sections:
    ...
    bar_chart({k: averages[k] for k in ["PTS","REB","AST","FG3M"] if k in averages},
              f"{title} — Key Averages")
